...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('<i><b>നിങ്ങൾ ആവശ്യപ്പെട്ട മൂവി എനിക്ക് എന്റെ ഡാറ്റാബേസിൽ നിന്നും കണ്ടെത്താൻ കഴിഞ്ഞില്ല ഈ മൂവി ഗ്രൂപ്പിൽ ആഡ് ചെയ്യാൻ വേണ്ടി മുകളിൽ കാണിച്ചതുപോലെ അഡ്മിൻ  മെൻഷൻ ചെയ്യുക📮 Mᴇɴᴛɪᴏɴ ➪  @admin Movie Name!    eg:-  @admin kgf 2 or Contact owner @Master_brooi</b></i>')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    elif query.data == "sticercomm":
        buttons = [[
            InlineKeyboardButton('𝙱𝙰𝙲𝙺', callback_data='stickerg')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.STICKERCOMMANDS,
            reply_markup=reply_markup,
            parse_mode='html'
        )  
